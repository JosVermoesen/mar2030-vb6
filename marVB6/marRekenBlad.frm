...
Private Sub cbXML_Click()
    ...
    bstNaam = Dir(BedrijfsLokatie & "*." & strType)
    If bstNaam = "" Then
...
Private Sub cmdBewaren_Click()
    ...
    If MsgBox(cbBestandsNaam.Text & " bewaren.  Bent U zeker ?", vbExclamation + vbYesNo + vbDefaultButton2) = vbYes Then
        If UCase(Right(cbBestandsNaam.Text, 3)) = "XML" Then
            KtrlBox = ScrMaakXMLTekstBestand(Spreadsheet1.XMLData, cbBestandsNaam.Text)
        Else
            KtrlBox = ScrMaakXMLTekstBestand(Spreadsheet1.HTMLData, cbBestandsNaam.Text)
        End If
...
Private Sub cmdInladen_Click()
    ...
    KtrlBox = ScrLeesXMLTekstBestand(TekstZelf, cbBestandsNaam.Text, True)
    If KtrlBox = False Then
...
Private Sub cmdZoeken_Click()
    ...
    If Ktrl Then
    Else
        txtIdCode.Text = rsMAR(FlKlant)("A110").Value
        txtNaam.Text = rsMAR(FlKlant)("A100").Value
        MsgBox "Stop"
        'RecordSamenStellen txtIdCode.Text, BoekjaarVanTot
    End If
...
Private Sub Form_Load()
    ...
    bstNaam = Dir(BedrijfsLokatie & "*.xml")
    If bstNaam = "" Then
...
Function ScrMaakXMLTekstBestand(DataZelf As Variant, Bestandsnaam As String) As Boolean

    On Error Resume Next
    Dim A As TextStream
    Screen.MousePointer = vbHourglass
    'Set A = fs.CreateTextFile(App.path & "\" & Bestandsnaam, True)
    Set A = fs.CreateTextFile(BedrijfsLokatie & Bestandsnaam, True)
    A.WriteLine (DataZelf)
    A.Close
    If Err Then
        ScrMaakXMLTekstBestand = False
    Else
        ScrMaakXMLTekstBestand = True
    End If
    A = Nothing
...
Function ScrLeesXMLTekstBestand(DataZelf As Variant, Bestandsnaam As String, LeesAlles As Boolean) As Boolean
    
    On Error Resume Next
    Const ForReading = 1
    Dim A As TextStream
    
    'Set A = fs.OpenTextFile(App.path & "\" & Bestandsnaam, ForReading, False, TristateFalse)
    Set A = fs.OpenTextFile(BedrijfsLokatie & Bestandsnaam, ForReading, False, TristateFalse)
    If Err Then ScrLeesXMLTekstBestand = False: Exit Function
    ...
    A.Close
    If Err Then
        ScrLeesXMLTekstBestand = False
    Else
        ScrLeesXMLTekstBestand = True
    End If
    A = Nothing
